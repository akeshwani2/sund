...
const Chat = (props: Props) => {
  ...
  // Add a ref to track the latest chat state
  const chatsRef = useRef(chatThread?.chats || []);
  useEffect(() => {
    chatsRef.current = chatThread?.chats || [];
  }, [chatThread?.chats]);
  ...
  // Memoize handlers with empty dependencies using refs
  const handleSearch = useCallback(async (chatIndex: number) => {
    const chat = chatsRef.current[chatIndex];
    setIsLoading(true);
  ...
  // Extract complex dependency checks into variables
  const lastChatMode = chatThread?.chats[chatThread?.chats.length - 1]?.mode;
  const lastChatSearchResults = chatThread?.chats[chatThread?.chats.length - 1]?.searchResults;
  const lastChatAnswer = chatThread?.chats[chatThread?.chats.length - 1]?.answer;
